...
const saveToLocalStorage = (key: string, data: any) => {
  localStorage.setItem(`skillbounty_${key}`, JSON.stringify(data));
};

const loadFromLocalStorage = (key: string, defaultValue: any) => {
  const item = localStorage.getItem(`skillbounty_${key}`);
  ...
  return {
    ...
    connectWallet: async (type, simulatedAddress) => {
      try {
        ...
        const challenge = `SkillBounty Authentication Challenge: ${Date.now()}`;
